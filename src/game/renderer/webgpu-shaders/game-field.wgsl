...
@group(0) @binding(1) var<storage, read> levelStorage: array<u32>; 
...
fn getField(field: vec2i) -> u32 {
  ...
  if (uniforms.playerAlive == 1.0 && 
    field.x == i32(uniforms.playerPosition.x) &&
    field.y == i32(uniforms.playerPosition.y)) {
    return 7 + u32(uniforms.playerDirection);
  }
...
fn fragmentMain(input: VertexOutput) -> @location(0) vec4f {
  ...
  color = step(1, f32(tile)) * 
    textureSample(spriteTexture, spriteSampler, spriteOffset);
  color.a = 1;
